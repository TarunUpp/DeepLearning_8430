...
if __name__ == "__main__":
    ...
    tf.app.flags.DEFINE_integer('sample_size',1240, 'train data sample')
    ...
    tf.app.flags.DEFINE_integer('max_encoder_steps', 32, 'Maximum encoder steps')
    tf.app.flags.DEFINE_integer('max_decoder_steps', 8, 'Maximum decoder steps')
    ...
    print ('Printing the generated file:')
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            for batch_start, batch_end in zip(range(0, FLAGS.sample_size, FLAGS.batch_size), range(FLAGS.batch_size, FLAGS.sample_size, FLAGS.batch_size)):
                print ("Trained batch:%04d/%04d" %(batch_end, FLAGS.sample_size))
            ...
            print("BLEU:average - " + str(avg))

            if (len(top_BLEU) < num_top_BLEU):
                top_BLEU.append(avg)
                print ("Final BLEU: %.4f ..." %(avg))
            ...
            print ("BLEU:[%d]:Max " %(num_top_BLEU), ["%.4f" % x for x in top_BLEU])

            print ("Epoch: # %d" %(epoch))
            print("Average BLEU: %.4f" %(avg))
            print ("Loss: %.4f" %(loss))
